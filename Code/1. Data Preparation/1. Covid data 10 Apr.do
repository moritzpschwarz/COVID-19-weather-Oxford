...
keep latitude longitude country province location_ID geo_resolution date* sum* travel*
...
* Expand the geo_resolution
encode geo_resolution, gen(geo)
bysort location_ID: egen min_geo = min(geo)
bysort location_ID: egen max_geo = max(geo)

* if we have no information on travel history, we set the binary variable to 0
replace travel_history_binary = 0 if missing(travel_history_binary)
...
save "C:\Users\morit\OneDrive - Nexus365\Covid-19 Paper\Data\Covid_19_extended_geofillin_08042020.dta", replace
