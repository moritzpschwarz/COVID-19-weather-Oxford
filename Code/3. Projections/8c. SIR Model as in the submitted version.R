...
climate_projection_df %>% 
  distinct(r0) -> model_assumptions


for(j in 1:length(model_assumptions$r0)){
  print(j)
  climate_projection_df %>% 
    ungroup %>% 
    select(Country,date,N,I,r0,type,climate_projection) %>% 
    mutate(type = factor(type,levels = c("Unmitigated","Enhanced social distancing of elderly","Social distancing whole population")),
           ...
           N=NULL) %>% 
    filter(r0==model_assumptions$r0[j]) %>% 
    gather(variable,value,-Country,-date,-r0,-type) %>% 
    ...
    labs(x = "", y = "Share of Population Infected", title = "2019-nCovid in a highly stylized SIR Model", 
         #subtitle = "Daily new cases in % of the population", 
         caption  = bquote("Using"~r[0]~"="~.(model_assumptions$r0[j]))) +
  ...
  ggsave(plot,filename = paste0("C:/Users/morit/OneDrive - Nexus365/Covid-19 Paper/Figures/Level Projections_r0",model_assumptions$r0[j],".pdf"),device = cairo_pdf,height = 8,width = 10)
  ggsave(plot,filename = paste0("C:/Users/morit/OneDrive - Nexus365/Covid-19 Paper/Figures/Level Projections_r0",model_assumptions$r0[j],".jpg"),height = 8,width = 10)
}