...
climate_projection_df %>% 
  ...
  mutate(climate_projection = accumulate(combined_growth[-n()], `*`, .init = first(climate_projection)),
         ...
         abs_growth = climate_projection - lag(climate_projection),
         abs_growth = ifelse(is.na(abs_growth),0,abs_growth),
         abs_growth = ifelse(abs_growth<0,0,abs_growth),
         S_new = N-cumsum(abs_growth)) %>% 
  na.omit()-> climate_projection_df

climate_projection_df %>% 
  mutate(Effect_adjusted = Effect*(S_new/N),
...
for(j in 1:length(model_assumptions$R0_input)){
  ...
  ggsave(plot,filename = paste0("C:/Users/morit/OneDrive - Nexus365/Covid-19 Paper/Figures/Level Projections_R0",model_assumptions$R0_input[j],"_Apr15_adjusted.pdf"),device = cairo_pdf,height = 8,width = 10)
  ggsave(plot,filename = paste0("C:/Users/morit/OneDrive - Nexus365/Covid-19 Paper/Figures/Level Projections_R0",model_assumptions$R0_input[j],"_Apr15_adjusted.jpg"),height = 8,width = 10)
}
